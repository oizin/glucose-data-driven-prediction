...
dir.create('output/graphs/spectral/heartrate/')
dir.create('output/graphs/trends/heartrate/')
...
pout
loc = paste0("output/graphs/heartrate_sample_",Sys.Date(),".png")
...
plot_stc("heartrate",
         ...
         span=0.2,
         p1='output/graphs/spectral/heartrate/',
         p2='output/graphs/trends/heartrate/',
         var_name="Heart rate (bpm)",
         ylim_cent = c(-30,30))

## E) TIME SERIES: BLOOD PRESSURE (MAP) -------------------------------------------------------

dir.create('output/graphs/spectral/map/')
dir.create('output/graphs/trends/map/')
...
pout
loc = paste0("output/graphs/map_sample_",Sys.Date(),".png")
ggsave(loc,plot = pout)
...
plot_stc("meanarterialpressure",
         ...
         span=0.3,
         p1='output/graphs/spectral/map/',
         p2='output/graphs/trends/map/',
         var_name="MAP (mmHg)",
         ylim_cent = NULL)

## F) TIME SERIES: TEMPERATURE -------------------------------------------------------

dir.create('output/graphs/spectral/temp/')
dir.create('output/graphs/trends/temp/')
...
pout = mimic_sugar[tmp_msk & icustay_id %in% samp4 & dy < 5,] %>%
  ...
  facet_wrap(~icustay_id,scales="free",ncol=2) 
pout
loc = paste0("output/graphs/temp_sample_",Sys.Date(),".png")
ggsave(loc,plot = pout)
...
plot_stc("temperature",
         ...
         span=1,
         p1='output/graphs/spectral/temp/',
         p2='output/graphs/trends/temp/',
         var_name="Temperature (Celcius)",
         ylim_cent = NULL)

## D) TIME SERIES: RESP RATE -------------------------------------------------------

dir.create('output/graphs/spectral/resprate/')
dir.create('output/graphs/trends/resprate/')
...
pout = mimic_sugar[resp_msk & icustay_id %in% samp4 & dy < 5,] %>%
  ...
  facet_wrap(~icustay_id,scales="free",ncol=2) 
pout
loc = paste0("output/graphs/rr_sample_",Sys.Date(),".png")
ggsave(loc,plot = pout)
...
plot_stc("resprate",
         ...
         span=0.5,
         p1='output/graphs/spectral/resprate/',
         p2='output/graphs/trends/resprate/',
         var_name="Respiratory rate (bpm)",
         ylim_cent = NULL)

## D) TIME SERIES: SPO2 -------------------------------------------------------

dir.create('output/graphs/spectral/spo2/')
dir.create('output/graphs/trends/spo2/')
...
pout = mimic_sugar[spo2_msk & icustay_id %in% samp4 & dy < 5,] %>%
  ...
  facet_wrap(~icustay_id,scales="free",ncol=2) 
pout
loc = paste0("output/graphs/spo2_sample_",Sys.Date(),".png")
ggsave(loc,plot = pout)
...
plot_stc("spo2",
         ...
         span=0.5,
         p1='output/graphs/spectral/spo2/',
         p2='output/graphs/trends/spo2/',
         var_name="Oxygen saturation (%)",
         ylim_cent = NULL)

## D) TIME SERIES: BLOOD PRESSURE (SYS/DIA) -----------------------------------------------

dir.create('output/graphs/spectral/bp/')
dir.create('output/graphs/trends/bp/')
...
pout = mimic_sugar[bp_msk & icustay_id %in% samp4 & dy < 5,c("icustay_id","tos_dy","sysbp","diasbp")] %>%
  ...
  facet_wrap(~icustay_id,scales="free",ncol=2) 
pout
loc = paste0("output/graphs/bp_sample_",Sys.Date(),".png")
ggsave(loc,plot = pout)
...
plot_stc("value",
         bp_df,
         span=0.5,
         p1='output/graphs/spectral/bp/',
         p2='output/graphs/trends/bp/',
