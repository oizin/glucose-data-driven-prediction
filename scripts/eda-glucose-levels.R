...
pout
loc = paste0("plots/glucose_sample_",Sys.Date(),".png")
...
for (i in 1:length(samp100)) {
  ...
  ggsave(filename = paste0('plots/spectral/glucose/spec_',samp100[i],'.png'),plot = pout)
  n <- length(pat_data)
  pout <- ggplot(pat_data) +
    ...
    theme_bw() +
    labs(title = samp100[i])
  ggsave(filename = paste0('plots/trends/glucose/trend_',samp100[i],'.png'),plot = pout)
}
...
pout = ggplot(av_glucose_t) +
  ...
  labs(y = "Blood glucose (mg/dL)", x = "Time in ICU (days)")
pout
loc = paste0("plots/glucose_all_",Sys.Date(),".png")
ggsave(loc,plot = pout)
...
pout
loc = paste0("plots/glucose_all_tmt_",Sys.Date(),".png")
ggsave(loc,plot = pout)
...
pout
loc = paste0("plots/glucose_dist_",Sys.Date(),".png")
ggsave(loc,plot = pout)
...
pout
loc = paste0("plots/glucose_dist_tmt_",Sys.Date(),".png")
ggsave(loc,plot = pout)
